...
function MenusZhTw({ theme, showNav, path, closeNav, filterMenuItems }) {
  ...
  const { wordpress } = useStaticQuery(graphql`
    query {
      wordpress {
        ...
        menus(where: {location: EXPANDED___ZH_TC}) {
          nodes {
            id
            name
            slug
      ...
      >
        {wordpress.menus &&
          ...
          <li className="passive-nav-item">
            <Link to={convertLinkLocale('/zh_tc/about', i18n.language)} onClick={((e) => handleNavClick(e, convertLinkLocale('/about', i18n.language)))}>
              {t('nav-about')}
            </Link>
          </li>
        {!isAuthenticated() && (
