...
function SociallyConnected() {
  ...
  const converInstPost = post => {
    ...
    return {
      ...
      date: `${moment(new Date(+`${post.taken_at_timestamp}000`)).format(lang !== 'en' ? 'MMMM DD, YYYY' : 'MM月 DD, YYYY')}`,
    }
  }
                ...
                <div>
                  <SociallyConnectedItem
                    {...{
                      ...
                      date: `${moment(new Date(fbPost.created_time.slice(0, 16))).format(lang !== 'en' ? 'MMMM DD, YYYY' : 'MM月 DD, YYYY')}`,
                    }}
                  />
                </div>
              )}
