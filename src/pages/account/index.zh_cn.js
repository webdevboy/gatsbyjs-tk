...
function Account() {
  ...
  return (
    <Layout>
      <div className={cx('account-wrapper', { 'account--active': isAccountView })}>
        <h2 className="account__title">
          {t('auth:my-account')}
        </h2>
        <div className="account__toggles">
          <div className={cx('account__toggle', { 'account__toggle--active': isAccountView })} onClick={() => {setAccountView(true)}}>
            {t('account-overview')}
          </div>
          <div className={cx('account__toggle', { 'account__toggle--active': !isAccountView })} onClick={() => {setAccountView(false)}}>
            {t('rest-password')}
          </div>
        </div>
        <AccountComponent />
